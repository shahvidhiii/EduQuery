...
from .paths import (
    AUDIOS_DIR,
    CHROMA_DB_DIR as DEFAULT_CHROMA_DB_DIR,
...
_embeddings_file = os.getenv("EMBEDDINGS_FILE", str(DEFAULT_EMBEDDINGS_FILE))
EMBEDDINGS_FILE = _embeddings_file if os.path.isabs(_embeddings_file) else os.path.join(base_dir, _embeddings_file)
...
_chroma_dir = os.getenv("CHROMA_DB_DIR", str(DEFAULT_CHROMA_DB_DIR))
...
_video_dir = os.getenv("VIDEO_DIR", str(VIDEOS_DIR))
...
async def lifespan(app: FastAPI):
    ...
    if app_state["backend"] == "pickle":
        ...
        if not os.path.exists(EMBEDDINGS_FILE):
            print(f"  [X] Embeddings file not found: {EMBEDDINGS_FILE}")
            ...
            try:
                ...
                print(f"  [OK] Embeddings loaded: {len(df)} chunks")
            except Exception as e:
                print(f"  [X] Could not load embeddings: {e}")
...
if os.path.isdir(JSON_DIR):
    app.mount("/jsons", StaticFiles(directory=JSON_DIR), name="jsons")
...
@app.post("/api/admin/pipeline/process-single")
async def process_single_video_pipeline(filename: str):
    """
    Process a single video through the full pipeline:
    1. Extract audio (ffmpeg)
    2. Transcribe (Whisper -> JSON chunks)
    3. Ingest into ChromaDB (embeddings)
    """
    import subprocess
    import sys
    ...
    if not os.path.isfile(video_path):
        raise HTTPException(status_code=404, detail=f"Video file '{filename}' not found in {VIDEO_DIR}")
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Pipeline failed at step after {steps_completed}. Error: {e.stderr or e.stdout or str(e)}"
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Pipeline failed: {str(e)}")
...
async def cleanup_duplicates():
    """Run the duplicate cleanup script to remove duplicate audio/json files."""
    import subprocess
    import sys
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Cleanup failed: {e.stderr or e.stdout or str(e)}")
    except Exception as e:
